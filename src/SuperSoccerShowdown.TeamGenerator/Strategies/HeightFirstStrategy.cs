...
public class HeightFirstStrategy : ITeamStrategy
{
    public StrategyType Type => StrategyType.HeightFirst;

    public List<PlayerDto> GenerateTeam(List<PlayerDto> players, int numberOfOffencePlayers)
    {
        var orderedPlayers = players.OrderByDescending(x => x.Height)
              .ThenBy(x => x.Weight)
              .ToList();
